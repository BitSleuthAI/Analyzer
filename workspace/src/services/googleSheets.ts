...
export async function appendToSheet(feedbackData: FeedbackOutput): Promise<void> {
  const { category, summary, sentiment, originalFeedback } = feedbackData;

  const spreadsheetId = process.env.GOOGLE_SHEETS_ID;
  const client_email = process.env.GOOGLE_SHEETS_CLIENT_EMAIL;
  // The private key is read directly from the environment variables.
  // The .env file should contain the key as a multi-line string enclosed in double quotes.
  const private_key = process.env.GOOGLE_SHEETS_PRIVATE_KEY;

  // Diagnostic logging to check if environment variables are loaded
  console.log("SHEETS: Attempting to append. Spreadsheet ID found:", !!spreadsheetId);
  console.log("SHEETS: Client Email found:", !!client_email);
  console.log("SHEETS: Private Key found:", !!private_key);

  if (!spreadsheetId || !client_email || !private_key) {
    throw new Error('Google Sheets API credentials are not fully configured. Please set GOOGLE_SHEETS_ID, GOOGLE_SHEETS_CLIENT_EMAIL, and GOOGLE_SHEETS_PRIVATE_KEY in your .env file.');
  }
